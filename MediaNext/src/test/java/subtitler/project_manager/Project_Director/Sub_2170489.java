 ...
 * 
 * @author pvaidya
 *Summary:Project Director Submission - Assignee are not showing fully in Compare Versions dialog
 *
 */

public class Sub_2170489 {
	
	Boolean assertion = true;
	LinkedHashMap<String, String> dataSet = new LinkedHashMap<String, String>();

	
	String SubmissionName = "SUB_2170489"+CommonElements.BROWSER+"N6";
	...
	public void setup() throws Exception {
		General.action().startSystem("SUB_2170489");
		dataSet.put("TL_test_case_description", "PD integration");
		dataSet.put("TL_internal_testCase_ID", "2170489");
	}
...
//				Thread.sleep(20000);
				...
				General.action().Click(PD_Pm_User_Submission_Locators.Locator().PD_Submission_compare_button);
		    	Thread.sleep(4000);
		    	
		    	assertion = Verify.action().verifyElementPresent(PD_Pm_User_Submission_Locators.Locator().PD_Submission_compare_dialogHeaders("Target"), 5);
				if (assertion == false) {
					report("f","Assertion failed while verifying compare version header");
					
				}
				assertion = Verify.action().verifyElementPresent(PD_Pm_User_Submission_Locators.Locator().PD_Submission_compare_dialogHeaders("Step"), 5);
				if (assertion == false) {
					report("f","Assertion failed while verifying compare version header");
					
				}
				assertion = Verify.action().verifyElementPresent(PD_Pm_User_Submission_Locators.Locator().PD_Submission_compare_dialogHeaders("Status"), 5);
				if (assertion == false) {
					report("f","Assertion failed while verifying compare version header");
					
				}
				assertion = Verify.action().verifyElementPresent(PD_Pm_User_Submission_Locators.Locator().PD_Submission_compare_dialogHeaders("Assignee"), 5);
				if (assertion == false) {
					report("f","Assertion failed while verifying compare version header");
					
				}
				assertion = Verify.action().verifyElementPresent(PD_Pm_User_Submission_Locators.Locator().PD_Submission_compare_dialog_Versions(2,"German (Germany)"), 5);
				if (assertion == false) {
					report("f","Assertion failed while verifying compare version information");
					
				}
				assertion = Verify.action().verifyElementPresent(PD_Pm_User_Submission_Locators.Locator().PD_Submission_compare_dialog_Versions(3,"completed"), 5);
				if (assertion == false) {
					report("f","Assertion failed while verifying compare version information");
					
				}
				assertion = Verify.action().verifyElementPresent(PD_Pm_User_Submission_Locators.Locator().PD_Submission_compare_dialog_Versions(4,"Completed"), 5);
				if (assertion == false) {
					report("f","Assertion failed while verifying compare version information");
					
				}
				assertion = Verify.action().verifyElementPresent(PD_Pm_User_Submission_Locators.Locator().PD_Submission_compare_dialog_Versions(5,"glplay_tdc14_vendor_2@fakemail.com"), 5);
				if (assertion == false) {
					report("f","Assertion failed while verifying compare version information");
