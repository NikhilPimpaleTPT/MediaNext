...
public class Sub_653799 {
	
	Boolean assertion = true;

	LinkedHashMap<String, String> dataSet = new LinkedHashMap<String, String>();

	String SubmissionName = "SUB_653799"+CommonElements.BROWSER+"A7";
  ...
  File filepath1;
	...
	public void setup() throws Exception {
		General.action().startSystem("SUB_653799");
		dataSet.put("TL_test_case_description","SUB-653799:As a Reviewer, I need to watch the final result of a translated subtitle, so that I can evaluate the final rendering.");
		dataSet.put("TL_internal_testCase_ID", "653799");
	}
	...
	public void testCase(LinkedHashMap<String, String> dataSet)
		...
		try {
			...
			PM_user.action().CreateSubmisson_Step3_VideoSRTFile(CommonElements.action().FILE_ABSOLUTE_PATH+ "653799\\");
            ...
            assertion = FileUtils.readFileToString(filepath1,CommonElements.UTF_8).contains("Salut, ceci est un test !");
    		if (assertion == false) {
    		...
    		assertion = FileUtils.readFileToString(filepath1,CommonElements.UTF_8).contains("Hey this is a test!");
    		if (assertion == false) {
    			report("f","Assertion failed while verifying File content with String.");
    		}
...
//			File filepath = new File(System.getProperty("user.home")+ "\\Downloads\\");
			...
			for (String filename : filenames) {
              ...
              reader.close();
				...
				boolean Finalresult = FileUtils.readFileToString(filepath1,CommonElements.UTF_8).contains("Salut, ceci est un test !");
	...
	public void translate_onGoing_submission(String SubmissionName,String target, boolean complete, boolean back) throws Exception {
		...
		//SEGMENT:-1  TARGET= â€œSalut, ceci est un test !â€�
			...
			Thread.sleep(1000);
			General.action().type(TranslatorLocators.Locator().translator_TargetSegement_textarea(1),"Salut, ceci est un test !");
			Thread.sleep(1000);
			
	   //SEGMENT:-2  TARGET= â€œSalut, ceci est un test !â€�
			...
			Thread.sleep(1000);
			General.action().type(TranslatorLocators.Locator().translator_TargetSegement_textarea(2),"Salut, ceci est un test !");
			Thread.sleep(1000);
	...
	public void reviewer_onGoing_submission(String SubmissionName,String target, boolean complete, boolean back) throws Exception {
		...
		System.out.println("No of IDS--------" + listofIds1.size());
			...
			//SEGMENR:-2 MODIFIED TARGET = "Salut, ceci est un test !!!!!"
			...
			General.action().waitforelemenetpresent(ReviewerLocators.Locator().reviewer_ModifiedTargetSegement_textarea(2));
			Thread.sleep(1000);
			General.action().type(ReviewerLocators.Locator().reviewer_ModifiedTargetSegement_textarea(2),"Salut, ceci est un test !!!!!");
			Thread.sleep(1000);
			
		
			
			//SEGMENR:-4 MODIFIED TARGET = "Salut, ceci est un test !"
			...
			General.action().waitforelemenetpresent(ReviewerLocators.Locator().reviewer_ModifiedTargetSegement_textarea(4));
			Thread.sleep(1000);
			General.action().type(ReviewerLocators.Locator().reviewer_ModifiedTargetSegement_textarea(4),"Salut, ceci est un test !");
			Thread.sleep(1000);
